    ...
    if (!class_exists('YuspRecommendationEngine')) {
        ...
        class YuspRecommendationEngine
        {
            ...
            public static function registerYuspSettings()
            {
                ...
                register_setting('yusp-settings-group', 'YuspUseCustomTrackingCode');
                register_setting('yusp-settings-group', 'YuspCustomTrackingCode');
            ...
            { ?>
                ...
                <div>
                    <div class="main-logo">
                        <img src="<?php echo plugins_url().'/yusp-recommendation-engine/img/yuspify-logo-woocommerce.svg'; ?>"
                    ...
                    <form method="post" action="options.php">
                        ...
                        <script> var useCustomTrackingCodeClicked = function () {
                                document.getElementById("yuspCustomTrackingCodeTr").style.display = document.getElementById("YuspUseCustomTrackingCode").checked ? "table-row" : "none";
                            }</script>
                        <table class="form-table">
                            ...
                            <tr>
                                <th>
                                    <label for="YuspUseCustomTrackingCode">Use custom tracking code url</label>
                                </th>
                                <td>
                                    <label class="switch">
                                        <input type="checkbox"
                                               name="YuspUseCustomTrackingCode"
                                               id="YuspUseCustomTrackingCode"
                                               onclick="useCustomTrackingCodeClicked()"
                                            <?php echo get_option('YuspUseCustomTrackingCode') ? "checked='checked'" : ""; ?>/>
                                        <span class="slider round"></span>
                                    </label>
                                </td>
                            </tr>
                            <tr id="yuspCustomTrackingCodeTr"
                                style="display: <?php echo get_option('YuspUseCustomTrackingCode') != 'on' ? "none" : "table-row"; ?>;">
                                <th>
                                    <label for="YuspCustomTrackingCode">Custom tracking code</label>
                                </th>
                                <td>
                                    <input placeholder="Insert your url for gr_reco"
                                           type="text"
                                           name="YuspCustomTrackingCode"
                                           id="YuspCustomTrackingCode"
                                           value="<?php echo esc_attr(get_option('YuspCustomTrackingCode')); ?>"/>
                                </td>
                                <td><?php
                                    if (get_option('YuspCustomTrackingCode') != '' &&
                                        !filter_var(get_option('YuspCustomTrackingCode'), FILTER_VALIDATE_URL)
                                    ) {
                                        echo '<p id="error">Please add a valid url!</p><br>';
                                    }
                                    ?></td>
                            </tr>
                        </table>
                    ...
                    <table class="form-table">
                        ...
                        <tr>
                            <th>
                                <form method="post" action="">
                                    <input type="submit" value="Get Server Side Access" name="serverSideAccess"
                                           class="button button-primary">
                                </form>
                            </th>
                            <td>
                    ...
                    if (isset($_POST['serverSideAccess'])) {
                        if ((get_option('customerId')) != '') {
                            ?>
                            <script>
                ...
                if (get_option('webshopServletUrl') != '' && get_option('customerId') != '' && get_option('password') != '') {
                    ...
                    <script>
                        (function (g, r, a, v, i, t, y) {
                            ...
                            y.async = 1;
                            y.src = <?php echo (get_option('YuspUseCustomTrackingCode') == 'on' &&
                                filter_var(get_option('YuspCustomTrackingCode'), FILTER_VALIDATE_URL)) ?
                                ("\"".get_option('YuspCustomTrackingCode')."\"") :
                                ("'//' + i + '/js/' + t + '/gr_reco5.min.js'"); ?>;
            ...
            public static function yuspGenerateItemsToXML($itemIds, &$file_contents)
            {
                for ($i = 0; $i < count($itemIds); $i++) {
                    ...
                    if ($post_type == 'product' || $post_type == 'product_variation') {
                        ...
                        $itemId = $itemIds[$i];
                        $sku = $product->get_sku();
                        $hidden = !$product->is_visible() ? 'true' : 'false';
                ...
                if (get_option('webshopServletUrl') != '' && get_option('customerId') != '' && get_option('password') != '') {
                    if (get_option('autoProductImport') != '') {
                        if (get_post_type($post_id) == 'product' || get_post_type($post_id) == 'product_variation') {
                            ...
                            YuspRecommendationEngine::yuspUpdateProduct($post_id, !$product->is_visible());
                        }
                    }
                }
            }
                ...
                for ($j = 0; $j < $items_count; $j++) {
                    ...
                    if ($post_type == 'product' || $post_type == 'product_variation') {
                        ...
                        if ($product != false &&
                            ...
                            $item->title = $title;
                            $item->hidden = !$product->is_visible();
